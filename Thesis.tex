...
Chalmers (1990) demonstrated that evolutionary processes can produce artificial systems that learn \cite{Chalmers:1990aa}.
...
In this way, nurturing may increase the competitive viability of learning by protecting the learner from otherwise costly errors made before proper behavior has been acquired \cite{Woehrer:2012aa}.
...
Genetic algorithms were developed by Holland (1975) to explore using artificial evolution as an optimization process \cite{Holland:1975aa}.
Many others have built on and applied Holland's ideas for genetic algorithms over the intervening decades,
and several other forms of evolutionary computation have since been developed \cite{Engelbrecht:2007aa}.
...
Many others have expanded upon this neural model and have developed a number of different neural models \cite{Engelbrecht:2007aa}.
...
however, this work is based closely on the work of Chalmers which considered single neurons in isolation \cite{Chalmers:1990aa}.
...
Chalmers \cite{Chalmers:1990aa} evolved learning rules by encoding the parameters of a template formula as strings, demonstrating that the delta rule for supervised learning can be evolved in certain situations for artificial neurons in static environments.
...
McQuesten and Miikulainen (1997) evolved reinforcement learning by culling subpar individuals and having parent networks teach their offspring using backpropagation \cite{McQuesten:1997aa}.
Niv et al. (2002) demonstrated the evolution of reinforcement learning in uncertain environments for networks with a single binary output \cite{Niv:2002aa}.
Shah (2015) similarly demonstrated the evolution of reinforcement learning in changing environments for networks with large numbers of rational-valued outputs \cite{Shah:2015hs}.

%% Nurturing
\section{Nurturing}

\emph{Nurturing} is defined as ``the contribution of time, energy, or other resources by one individual to the expected physical, mental, social, or other development of another individual with which it has an ongoing relationship" \cite{Woehrer:2012aa}.

Nurturing is prevalent in the biological world and can be an important contributing factor to the evolution of learning \cite{Woehrer:2012aa} \cite{Eskridge:2012aa},
...
Leonce, Hoke, and Hougen (2012) demonstrated the evolution of instinctive nurturing in robots \cite{Leonce:2012aa}.
...
The work of McQuesten and Miikkulainen (1997),
where it was shown that neuroevolution could benefit from parent networks teaching their offspring using backpropagation,
is the first known example demonstrating that nurturing can promote the evolution of learning \cite{McQuesten:1997aa};
...
Eskridge \& Hougen \cite{Eskridge:2012aa} conducted experiments involving food patch estimation in uncertain environments,
...
This result serves as a re-confirmation of Chalmers's original findings \cite{Chalmers:1990aa}.
...
and a reconfirmation of Chalmers's basic conclusions \cite{Chalmers:1990aa}.
