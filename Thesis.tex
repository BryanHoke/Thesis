...
Learning could also not emerge if none of the behavior adjustment rules evolved performed better than adjusting behavior randomly.
However, in Chalmers's work learning was the only viable phenotypic strategy that could emerge from evolution.
...
It examines also whether learning is less likely to evolve when both instincts and learning have the potential to emerge.
...
The chromosomes are evaluated by an objective function referred to as the \emph{fitness function} to determine the fitness value of each one.
